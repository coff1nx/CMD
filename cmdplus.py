...
class CmdPlus(QMainWindow):
    ...
    def handle_command(self, cmd_override=None):
        ...
        if clean_cmd.lower().startswith("cd "):
            try:
                path = clean_cmd[3:].strip()
                os.chdir(path)
                self.current_dir = os.getcwd()
        ...
        if clean_cmd.lower().endswith(".py") and os.path.isfile(clean_cmd):
            cmd = f'python -u "{clean_cmd}"'

        
        if self.real_cmd_active:
            if self.real_session:
                self.real_session.send_command(cmd)
            return

        
        self.terminal.insertPlainText(f"\n> {cmd}\n")

        if ".py" in cmd.lower() or os.path.isfile(clean_cmd):
    ...
    def print_initial_info(self):
        self.terminal.appendPlainText(f"cmdplus coff1nx ready | {self.admin_status}")
